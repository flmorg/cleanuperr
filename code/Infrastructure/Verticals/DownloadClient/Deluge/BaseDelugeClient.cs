...
using Domain.Deluge.Exceptions;
using Domain.Deluge.Request;
...
public abstract class BaseDelugeClient
{
    protected readonly DelugeConfig _config;
    protected readonly HttpClient _httpClient;
    
    public BaseDelugeClient(IOptions<DelugeConfig> config, IHttpClientFactory httpClientFactory)
    {
        _config = config.Value;
        _httpClient = httpClientFactory.CreateClient(nameof(DelugeClient));
    }
    ...
    private async Task<String> PostJson(String json)
    {
        ...
        var responseMessage = await _httpClient.PostAsync(_config.Url, content);
    ...
    private async Task<T> SendRequest<T>(string method, params object[] parameters)
    {
        return await SendRequest<T>(CreateRequest(method, parameters));
    }

    private async Task<T> SendRequest<T>(DelugeRequest webRequest)
    {
